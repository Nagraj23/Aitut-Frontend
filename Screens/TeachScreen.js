...
const TeachScreen = ({ navigation, route }) => {
    ...
    const chatKey = `chat_${subject}_${day}_${topic}`;

    /* ---------------- INIT (FRESH CHAT ALWAYS) ---------------- */
    useEffect(() => {
        const prepareSession = async () => {
            try {
                ...
                if (detailString) {
                    ...
                    if (parsed?.id) setFullUser(parsed);
                }
    ...
    }, [ready, fullUser, messages]);

    /* ---------------- STREAM API ---------------- */
    const handleTeachService = (msgText) => {
        ...
        const univ = (fullUser?.university || "solapur").toLowerCase();
        const branch = (fullUser?.branch || "cse").toLowerCase();
        const year = fullUser?.year || "1";

        const url = `${AI_URL}/ask/${univ}/${branch}/${year}/${subject.toLowerCase()}/${day}`;
